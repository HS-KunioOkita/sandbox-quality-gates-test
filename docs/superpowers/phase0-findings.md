...
**追記（Phase 3、`run-all.sh` 全体の実測。申し送り #26 への決着）**：コントローラが `./verification/run-all.sh`（全 14 ケース + 対照実行）を実行し、**6 分 9 秒で完走した。** 申し送り #26 が「概ね 40 分」としていた Phase 2 時点の推定（ログの mtime と実行者の申告だけが根拠で、厳密な計測ではないと明記済み）から大きく外れている。
...
**Phase 5 で 19 ケースになっても、単純な比例計算で 10 分程度と見込める。** 申し送り #26 が挙げた高速化候補（semgrep のレジストリ取得のキャッシュ、Docker イメージの事前 pull、ケースの並列実行）は、**現時点の実測を前提にする限り不要**と判断できる。
