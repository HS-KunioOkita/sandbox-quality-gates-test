...
**しかし 3 回目の結果は、対照実行を足しただけでは説明できない。** ケース分だけを比べても 6 分 1 秒 → 13 分 43 秒で 2.3 倍になっている。**同じ 14 ケース・同じキャッシュ状態・判定は完全に同一**（`RESULTS.md` はバイト単位で不変）だったので、差は検証対象の側ではなくマシンの負荷である。3 回目の実行中に計測した値:

- load average **13.89 / 16.71 / 13.71**
- Docker に割り当てられた 7.65 GiB のうち **2.65 GiB を無関係な別プロジェクトの devcontainer が占有**していた

負荷が引いた後に `L3-03-authz-bypass` 単体を計測し直すと 39 秒で、3 回目の同ケース（89 秒）ではなく初回（28 秒）の側に戻った。
...
- **ゲート別の内訳の構造**（負荷に依らず `l2-semgrep` が支配的で、その大半はレジストリからの 147 ルール取得である）。ここはキャッシュの効果が読める。
...
| 26 | `run-all.sh` の所要時間 | **計測手段は解消・判断は未決着**（Task 8 + 最終レビュー F5(a)）。経過秒数を stderr に出す実装を入れ、計測開始位置を対照実行の前へ移した。実測は 3 回で **6 分 9 秒 / 6 分 1 秒（ケース分のみ）/ 14 分 35 秒（対照実行込み）**。**同一条件で 2.3 倍の幅が出るため、高速化が要るかどうかは壁時計では決められない。** #26 の高速化候補（semgrep のキャッシュ、Docker の事前 pull、並列化）の要否は Phase 4 以降へ持ち越す。§1.38 |
...
| `./verification/run-all.sh` | 対照実行が通り、14 ケースすべて判定。**3 回実行し、判定は 3 回とも同一**（最終レビューの修正波の後も `RESULTS.md` はバイト単位で不変）。所要は 6 分 9 秒 / 6 分 1 秒（ケース分のみ）/ 14 分 35 秒（対照実行込み）で、**壁時計は同一条件でも 2.3 倍の幅が出る**（§1.38） |
| `./verification/run-all.sh` 実行後の状態 | 作業ツリークリーン、`verify/*` ブランチ残存なし |
