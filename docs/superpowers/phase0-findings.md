...
| 16 | `l2-install.sh` がツールの実行失敗を fail(1) に写像している | **解消**（Task 7）。ログの理由コードで fail を絞り、ネットワーク断は error(2) に落とした |
...
| 内容 | 仕分け |
|---|---|
| `l2-osv.sh` のコメントは「lockfile 不在 / ネットワーク断 / イメージ起動失敗は 1 以外の非ゼロになる」と主張しているが、**実測したのは脆弱性検出時の 1 だけ**。他の分岐は未実測 | **Phase 3 で実測するか、コメントを「未実測」と正直に書き換える。** 予防的に書いた推測が実測のように読める状態は、この検証の規律に反する |
...
| `run-case.sh` が `l2-new-deps` をリテラルパスで呼んでおり、`GATE_DETECTION` 配列がまだどこからも消費されていない | **非ブロックゲートが 2 本目になる時点で対処する。** 現状は 1 本なので配列を回す実装のほうが冗長 |
...
| `.gitleaks.toml` のコメントに Task 3 由来の言い回しが残り、「実測」と書いた箇所の一部は正規表現の構成上自明（`.diff` は `\.md$` に一致しえない） | **Phase 3 で文言を直す。** 上の 1 件目と同じ理由 |
