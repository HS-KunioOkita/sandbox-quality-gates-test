...
>
> **Phase 3 着手時の決定：(a) を選択した。** `GET /orders/:id` を追加し、他人の注文には 403 を返す所有者チェックを入れる。`case.patch` は `@UseGuards` を残したまま**所有者チェックだけを外す**。手順書 §3.2 の Semgrep カスタムルールは Controller にガードが付いているかしか見ないため発火しない見込みで、`L2-02` と欠陥の型が重ならない。
>
> したがって本ケースの `claimed_layer` は**手順書 §10 の主張に従って `L2`** とする（§10 は「認可チェックの欠落 → L2 / L5」）。Semgrep が発火せず L3 の e2e が捕まえた場合、それは §10 の割り当てが「ガードの有無」しか対象にできていないことへの反証データになる。
>
> 403 と 404 では 404 のほうがセキュリティの定石だが、ここは検証対象である設計書・手順書の記述（「403 を返す経路を作る」）に寄せて 403 とする。
