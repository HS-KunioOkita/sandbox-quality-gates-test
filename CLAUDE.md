...
**ある層を足す作業が、別の層のゲートを赤くする。** Phase 3 で 4 回起きた。L3 の依存追加（`@nestjs/swagger` / `openapi-typescript`）が js-yaml の High 脆弱性を持ち込んで `l2-osv` を赤くし（§1.34）、Playwright のテストファイルが vitest の既定 include に拾われて `l3-test` を壊し、同じファイルが ESLint の `projectService` で解決できず `l1-lint` を壊し（§1.36 / §1.37）、`L3-03` の欠陥注入が `l1-typecheck` と `l3-openapi-drift` に同時に当たってケースを判定不能にした（§1.41）。**Phase 4 でさらに 2 回起きた。** Stryker の依存が `fast-uri`（High）と `qs`（Medium）を持ち込んで `l2-osv` を赤くし（§1.58）、Stryker の `json` / `html` レポートがミューテート対象のソース全文を埋め込むため、直前のケースが仕込んだ秘密が次のケースの `l2-gitleaks` に漏れた（§1.55）。**新しい層を足したら、既存の全ゲートを回すまで終わりではない。**

## 現在地

Phase 0（モノレポ基盤とサンプルアプリ）、Phase 1（L1 ゲート + 検証ハーネス + L1 系 6 ケース）、Phase 2（L2 ゲート 4 本 + L2 系 5 ケース）、Phase 3（L3 ゲート 2 本 + L3 系 3 ケース）、Phase 4（L4 ゲート 1 本 + L4 系 2 ケース）が完了。次は Phase 5（L5）。

ブロックするゲートは 9 本（`scripts/gates/gates.list.sh` の `GATE_ORDER`）+ 非ブロック 1 本。Playwright（`l3-e2e-web.sh`）は**意図的に `GATE_ORDER` の外**に置いてある（`phase0-findings.md` §1.35）。`apps/web` の Stryker も**意図的に `GATE_ORDER` の外**（nightly 用。§1.48）。

全 16 ケースの結果は `verification/RESULTS.md`（**✅ 10 行 / ❌ 6 行 / ⚠️ 0 行**）。既存 14 ケースは Phase 3 から退行なし（✅ 10 / ❌ 4）で、増えた ❌ 2 行は L4 系の 2 ケースである。L1 系は Phase 1 から退行なし（5 件 ✅ / L1-06 のみ ❌）。`run-all.sh` の所要は 4 回の実測で **6 分 9 秒 / 6 分 1 秒（いずれも 14 ケース分・対照実行を含まない）/ 14 分 35 秒（14 ケース + 対照実行）/ 8 分 7 秒（16 ケース + 対照実行 33 秒、ゲート 9 本）**。Phase 2 時点の「概ね 40 分」という推定は厳密な計測ではなかったが、**実測どうしの幅も 2.3 倍ある**（同じ 14 ケース・同じ判定で。§1.38）。**ゲートとケースが増えた Phase 4 の回のほうが Phase 3 の 3 回目より短い**——これは高速化ではなくマシン負荷の差である。**壁時計を根拠に判断しないこと。**
...
- `l4-mutation` は L1 系・L2 系のケースでも fail する（`L1-01` / `L2-03` / `L2-04`）。**欠陥を検出したのではなく、たまたま差分に入ったファイル（`orders.service.ts`、スコア 40 %）が閾値 50 を下回っているだけである。** 判定は ✅ のままだが、`expect` の pass/fail を層の独立性の証拠として読まないこと（§1.53）。
