...
```bash
./verification/run-case.sh <CASE-ID>   # 1 ケース（実測 21〜153 秒。§1.38）
./verification/run-all.sh              # 全ケース + 対照実行、RESULTS.md 生成（14 ケースで実測 6〜15 分。§1.38）
```

**`run-all.sh` は必ずバックグラウンドで実行すること。** Bash ツールのタイムアウト上限（10 分）を超える実行を実測している（14 分 35 秒）。

**所要時間の絶対値は再現しない。** 同じ 14 ケース・同じキャッシュ状態・判定も完全に同一（`RESULTS.md` はバイト単位で不変）なのに、6 分 1 秒の回と 13 分 43 秒の回がある。差はマシンの負荷（後者の実行中は load average 13.89、Docker の 7.65 GiB のうち 2.65 GiB を無関係な devcontainer が占有）であって、検証対象の側ではない。**壁時計を根拠に判断しないこと。** 使えるのはゲート別の相対的な内訳という構造のほうである（§1.38）。
...
全 14 ケースの結果は `verification/RESULTS.md`（**✅ 10 行 / ❌ 4 行**）。L1 系は Phase 1 から退行なし（5 件 ✅ / L1-06 のみ ❌）。`run-all.sh` の所要は 3 回の実測で **6 分 9 秒 / 6 分 1 秒（いずれも 14 ケース分・対照実行を含まない）/ 14 分 35 秒（対照実行込み）**。Phase 2 時点の「概ね 40 分」という推定は厳密な計測ではなかったが、**実測どうしの幅も 2.3 倍ある**（§1.38）。
