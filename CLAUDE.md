...
- **❌ の 6 行と「`claimVerdict` が `mismatch`」の 3 件は一致しない。** 内訳は `mismatch` 3 件（`L1-06` / `L2-05` / `L3-03`）、`not-caught` 2 件（`L4-01` / `L4-02`）、`match` 1 件（`L2-01-phantom-package`。層の主張は成り立っており、ケースが名指しした `l2-osv` だけが無反応で `claimGateVerdict` のみ `mismatch`。§1.19）。**件数を引くときはどちらの数字か明示すること**（§4 の Phase 4 の表に内訳がある）。
